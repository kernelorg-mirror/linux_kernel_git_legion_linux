...
static int decode_insn_struct(struct insn *insn, struct pt_regs *regs)
{
	...
	if (user_mode(regs)) {
		int nr_copied = insn_fetch_from_user_inatomic(regs, buffer);
		if (nr_copied <= 0)
			return -EFAULT;
...
static int get_phys_addr(unsigned long addr, phys_addr_t *phys_addr)
{
	unsigned int level;
	pgd_t *pgdp;
	pte_t *ptep;
			...
			unsigned long vaddr)
{
	...
	/* Check whether #VE matches the instruction that was decoded. */
	switch (mmio) {
	case INSN_MMIO_WRITE:
	case INSN_MMIO_WRITE_IMM:
		if (!(ve->exit_qual & EPT_VIOLATION_ACC_WRITE))
			return -EFAULT;
		break;
	case INSN_MMIO_READ:
	...
	if (get_phys_addr(vaddr, &phys_addr) || (ve->gpa != cc_mkdec(phys_addr)))
		return -EAGAIN;
