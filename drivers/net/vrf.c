...
static void *vrf_strict_mode_data(const struct ctl_context *ctx)
{
	return ctx->ns.net_ns;
}

static const struct ctl_field vrf_table[] = {
	CTL_FIELD_CUSTOM("strict_mode", 0644, 0, vrf_strict_mode_data,
			 vrf_shared_table_handler),
};

static int vrf_netns_init_sysctl(struct net *net, struct netns_vrf *nn_vrf)
{
	nn_vrf->ctl_hdr = register_net_sysctl_fields_sz(net, "net/vrf",
							vrf_table,
