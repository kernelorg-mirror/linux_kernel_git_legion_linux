...
#define BPF_CGROUP_RUN_PROG_SYSCTL(head, ctx, table, write, buf, count, pos)  \
({									       \
	int __ret = 0;							       \
	if (cgroup_bpf_enabled(CGROUP_SYSCTL))			       \
		__ret = __cgroup_bpf_run_filter_sysctl(head, ctx, table, write,\
						       buf, count, pos,        \
						       CGROUP_SYSCTL);     \
	__ret;								       \
})
...
#define BPF_CGROUP_RUN_PROG_SYSCTL(head,ctx,table,write,buf,count,pos) ({ 0; })
