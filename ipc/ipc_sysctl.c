...
static int proc_ipc_dointvec_minmax(const struct ctl_table *table, int write,
		void *buffer, size_t *lenp, loff_t *ppos)
{
	struct ctl_table tmp = *table;
...
static int proc_ipc_dointvec_minmax_mni(const struct ctl_table *table, int write,
		void *buffer, size_t *lenp, loff_t *ppos)
{
	struct ctl_table tmp = *table;
...
static type *ipc_ ## field ## _data(const struct ctl_context *ctx)	\
{									\
...
static int *ipc_ ## name ## _data(const struct ctl_context *ctx)	\
{									\
...
static const struct ctl_field ipc_sysctls[] = {
	CTL_FIELD_CUSTOM("shmmax", 0644, sizeof(init_ipc_ns.shm_ctlmax),
			 ipc_shm_ctlmax_data, proc_doulongvec_minmax),
	CTL_FIELD_CUSTOM("shmall", 0644, sizeof(init_ipc_ns.shm_ctlall),
			 ipc_shm_ctlall_data, proc_doulongvec_minmax),
	CTL_FIELD_STATIC_INT_MINMAX("shmmni", 0644, ipc_shm_ctlmni_data,
				    SYSCTL_ZERO, &ipc_mni),
	CTL_FIELD_CUSTOM("shm_rmid_forced", 0644,
			 sizeof(init_ipc_ns.shm_rmid_forced),
			 ipc_shm_rmid_forced_data,
			 proc_ipc_dointvec_minmax_orphans),
	CTL_FIELD_CUSTOM("msgmax", 0644, sizeof(init_ipc_ns.msg_ctlmax),
			 ipc_msg_ctlmax_data, proc_ipc_dointvec_minmax),
	CTL_FIELD_CUSTOM("msgmni", 0644, sizeof(init_ipc_ns.msg_ctlmni),
			 ipc_msg_ctlmni_data, proc_ipc_dointvec_minmax_mni),
	CTL_FIELD_CUSTOM("auto_msgmni", 0644, sizeof(int), NULL,
			 proc_ipc_auto_msgmni),
	CTL_FIELD_CUSTOM("msgmnb", 0644, sizeof(init_ipc_ns.msg_ctlmnb),
			 ipc_msg_ctlmnb_data, proc_ipc_dointvec_minmax),
	CTL_FIELD_CUSTOM("sem", 0644, 4 * sizeof(int), ipc_sem_ctls_data,
			 proc_ipc_sem_dointvec),
#ifdef CONFIG_CHECKPOINT_RESTORE
	CTL_FIELD_STATIC_INT_MINMAX("sem_next_id", 0444,
				    ipc_sem_next_id_data, SYSCTL_ZERO,
				    SYSCTL_INT_MAX),
	CTL_FIELD_STATIC_INT_MINMAX("msg_next_id", 0444,
				    ipc_msg_next_id_data, SYSCTL_ZERO,
				    SYSCTL_INT_MAX),
	CTL_FIELD_STATIC_INT_MINMAX("shm_next_id", 0444,
