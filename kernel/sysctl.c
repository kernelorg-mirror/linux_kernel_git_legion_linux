...
int do_proc_dobool_conv(bool *negp, unsigned long *lvalp,
			int *valp,
...
int do_proc_dointvec_minmax_conv(bool *negp, unsigned long *lvalp,
				 int *valp,
				 int write, int *min, int *max)
{
	if (write) {
...
int do_proc_douintvec_minmax_conv(unsigned long *lvalp,
				  unsigned int *valp,
...
ssize_t proc_dointvec_minmax_r(struct ctl_context *ctx, struct file *file,
			       char *buffer, size_t *lenp, loff_t *ppos)
{
	return do_proc_dointvec_r(ctx->ctl_table->data, ctx->ctl_table,
			buffer, lenp, ppos,
...
ssize_t proc_dointvec_minmax_w(struct ctl_context *ctx, struct file *file,
			       char *buffer, size_t *lenp, loff_t *ppos)
{
	return do_proc_dointvec_w(ctx->ctl_table->data, ctx->ctl_table,
			buffer, lenp, ppos,
			...
			unsigned int *min, unsigned int *max)
{
	...
	if (conv(&lval, tbl_data, 1, min, max)) {
		err = -EINVAL;
		goto out_free;
	}
...
out_free:
	...
	/* This is in keeping with old do_proc_dointvec() */
bail_early:
	*ppos += *lenp;
	return err;
}
			...
			unsigned int *min, unsigned int *max)
{
	...
	if (conv(&lval, tbl_data, 0, min, max)) {
		err = -EINVAL;
		goto out;
	}
...
{
	if (write)
		return do_proc_dointvec_w(table->data, table, buffer, lenp, ppos,
				do_proc_dobool_conv, NULL, NULL);
	return do_proc_dointvec_r(table->data, table, buffer, lenp, ppos,
			do_proc_dobool_conv, NULL, NULL);
}
		...
		int write, void *buffer, size_t *lenp, loff_t *ppos)
{
	...
	old = *(int *)table->data;
	ret = do_proc_dointvec_w(table->data, table, buffer, lenp, ppos,
...
/**
 * proc_dointvec_minmax_r - read a vector of integers
...
/**
 * proc_dointvec_minmax_w - read a vector of integers with min/max values
 * @ctx: the operation context which contains sysctl table
 * @file: the opened sysctl file
 * @buffer: the user buffer
 * @lenp: the size of the user buffer
			...
			void *buffer, size_t *lenp, loff_t *ppos)
{
	...
	val = *data;
	if (write)
		res = do_proc_douintvec_w(&val, &tmp, buffer, lenp, ppos,
					  do_proc_douintvec_minmax_conv, &min, &max);
	else
		res = do_proc_douintvec_r(&val, &tmp, buffer, lenp, ppos,
					  do_proc_douintvec_minmax_conv, &min, &max);
	if (res)
		return res;
	if (write)
				...
				void *buffer, size_t *lenp, loff_t *ppos)
{
	...
	if (write)
		ret = do_proc_dointvec_w(&tmp, table, buffer, lenp, ppos,
					 do_proc_dointvec_minmax_conv, NULL, NULL);
	else
		ret = do_proc_dointvec_r(&tmp, table, buffer, lenp, ppos,
					 do_proc_dointvec_minmax_conv, NULL, NULL);
		...
		size_t *lenp, loff_t *ppos)
{
	...
	if (write)
		r = do_proc_dointvec_w(&tmp, table, buffer, lenp, ppos,
				       do_proc_dointvec_minmax_conv, NULL, NULL);
	else
		r = do_proc_dointvec_r(&tmp, table, buffer, lenp, ppos,
				       do_proc_dointvec_minmax_conv, NULL, NULL);
...
/**
 * do_proc_large_bitmap_w - write to a large bitmap
 * @data: the bitmap
 * @table: the sysctl table
 * @buffer: the user buffer
 * @lenp: the size of the user buffer
 ...
 * Returns 0 on success.
 */
static int do_proc_large_bitmap_w(void *data, struct ctl_table *table,
		void *buffer, size_t *lenp, loff_t *ppos)
{
	int err = 0;
	size_t left = *lenp;
...
/**
 * do_proc_large_bitmap_r - read from a large bitmap
 * @data: the bitmap
 * @table: the sysctl table
 * @buffer: the user buffer
 * @lenp: the size of the user buffer
 ...
 * Returns 0 on success.
 */
static int do_proc_large_bitmap_r(void *data, struct ctl_table *table,
		void *buffer, size_t *lenp, loff_t *ppos)
{
	size_t left = *lenp;
	unsigned long bitmap_len = table->maxlen;
...
ssize_t proc_large_bitmap_r(struct ctl_context *ctx, struct file *file,
			    char *buffer, size_t *lenp, loff_t *ppos)
{
	return do_proc_large_bitmap_r(ctx->ctl_table->data, ctx->ctl_table,
				      buffer, lenp, ppos);
}
...
ssize_t proc_large_bitmap_w(struct ctl_context *ctx, struct file *file,
			    char *buffer, size_t *lenp, loff_t *ppos)
{
	return do_proc_large_bitmap_w(ctx->ctl_table->data, ctx->ctl_table,
				      buffer, lenp, ppos);
}
...
struct ctl_fops proc_dointvec_minmax_fops = {
	.read  = proc_dointvec_minmax_r,
	.write = proc_dointvec_minmax_w,
};

struct ctl_fops proc_dointvec_jiffies_fops = {
	.read  = proc_dointvec_jiffies_r,
	.write = proc_dointvec_jiffies_w,
};

struct ctl_fops proc_dointvec_userhz_jiffies_fops = {
	.read  = proc_dointvec_userhz_jiffies_r,
	.write = proc_dointvec_userhz_jiffies_w,
};

struct ctl_fops proc_dointvec_ms_jiffies_fops = {
	.read  = proc_dointvec_ms_jiffies_r,
	.write = proc_dointvec_ms_jiffies_w,
};

struct ctl_fops proc_douintvec_minmax_fops = {
	.read  = proc_douintvec_minmax_r,
	.write = proc_douintvec_minmax_w,
};

struct ctl_fops proc_large_bitmap_fops = {
	.read = proc_large_bitmap_r,
	.write = proc_large_bitmap_w,
};
		       ...
		       void *buffer, size_t *lenp, loff_t *ppos)
{
	...
	if (!write) {
		ret = do_proc_dointvec_r(&val, table, buffer, lenp, ppos,
				do_proc_dointvec_minmax_conv, NULL, NULL);
		goto ret;
	}
	ret = do_proc_dointvec_w(&val, table, buffer, lenp, ppos,
...
EXPORT_SYMBOL(do_proc_dobool_conv);
EXPORT_SYMBOL(proc_dointvec);
EXPORT_SYMBOL(proc_dointvec_jiffies);
EXPORT_SYMBOL(proc_dointvec_jiffies_r);
EXPORT_SYMBOL(proc_dointvec_jiffies_w);
EXPORT_SYMBOL(proc_dointvec_jiffies_fops);
EXPORT_SYMBOL(proc_dointvec_userhz_jiffies_r);
EXPORT_SYMBOL(proc_dointvec_userhz_jiffies_w);
EXPORT_SYMBOL(proc_dointvec_userhz_jiffies_fops);
EXPORT_SYMBOL(proc_dointvec_ms_jiffies_r);
EXPORT_SYMBOL(proc_dointvec_ms_jiffies_w);
EXPORT_SYMBOL(proc_dointvec_ms_jiffies_fops);
EXPORT_SYMBOL(proc_dointvec_minmax_r);
EXPORT_SYMBOL(proc_dointvec_minmax_w);
EXPORT_SYMBOL(do_proc_dointvec_minmax_conv);
EXPORT_SYMBOL(do_proc_dointvec_r);
EXPORT_SYMBOL(do_proc_dointvec_w);
EXPORT_SYMBOL(proc_dointvec_minmax_fops);
EXPORT_SYMBOL(do_proc_douintvec_minmax_conv);
EXPORT_SYMBOL(do_proc_douintvec_r);
EXPORT_SYMBOL(do_proc_douintvec_w);
EXPORT_SYMBOL(proc_douintvec_minmax_fops);
EXPORT_SYMBOL(proc_douintvec_minmax_r);
EXPORT_SYMBOL(proc_douintvec_minmax_w);
EXPORT_SYMBOL(proc_dointvec_userhz_jiffies);
EXPORT_SYMBOL(proc_dointvec_ms_jiffies);
EXPORT_SYMBOL(proc_dostring);
EXPORT_SYMBOL(proc_doulongvec_minmax);
EXPORT_SYMBOL(proc_doulongvec_ms_jiffies_minmax);
EXPORT_SYMBOL(proc_large_bitmap_fops);
EXPORT_SYMBOL(proc_large_bitmap_r);
EXPORT_SYMBOL(proc_large_bitmap_w);
