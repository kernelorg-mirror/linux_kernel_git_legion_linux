				...
				void *buffer, size_t *lenp, loff_t *ppos)
{
	...
	if (!write) {
		...
		*param = (watchdog_enabled & which) != 0;
		err = sysctl_read_intvec_data(param, table, buffer, lenp, ppos,
				sysctl_conv_intvec, table->extra1, table->extra2);
	} else {
		old = READ_ONCE(*param);
		err = sysctl_write_intvec_data(param, table, buffer, lenp, ppos,
				sysctl_conv_intvec, table->extra1, table->extra2);
		...
		char *buffer, size_t *lenp, loff_t *ppos)
{
	...
	err = sysctl_read_intvec(ctx, file, buffer, lenp, ppos);
