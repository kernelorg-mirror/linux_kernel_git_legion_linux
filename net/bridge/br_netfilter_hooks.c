...
#define BRNF_DATA(name, expr)						\
static void *brnf_ ## name ## _data(const struct ctl_context *ctx)	\
{									\
	struct brnf_net *brnet = net_generic(ctx->ns.net_ns, brnf_net_id); \
	return (expr);							\
}

BRNF_DATA(call_arptables, &brnet->call_arptables)
BRNF_DATA(call_iptables, &brnet->call_iptables)
BRNF_DATA(call_ip6tables, &brnet->call_ip6tables)
BRNF_DATA(filter_vlan_tagged, &brnet->filter_vlan_tagged)
BRNF_DATA(filter_pppoe_tagged, &brnet->filter_pppoe_tagged)
BRNF_DATA(pass_vlan_indev, &brnet->pass_vlan_indev)

static const struct ctl_field brnf_table[] = {
	{
		.table = {
	...
	},
	{
		...
		.data = brnf_filter_vlan_tagged_data,
	},
	{
		.table = {
		...
		.data = brnf_filter_pppoe_tagged_data,
	},
	{
		.table = {
		...
		.data = brnf_pass_vlan_indev_data,
	},
};
...
static int br_netfilter_sysctl_init_net(struct net *net)
{
	...
	brnet->ctl_hdr = register_net_sysctl_fields(net, "net/bridge",
						    brnf_table,
...
static void br_netfilter_sysctl_exit_net(struct brnf_net *brnet)
{
...
#ifdef CONFIG_SYSCTL
	br_netfilter_sysctl_exit_net(brnet);
#endif
}
