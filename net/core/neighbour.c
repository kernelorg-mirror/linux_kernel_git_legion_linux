		...
		struct file *file, char *buffer, size_t *lenp, loff_t *ppos)
{
	...
	ret = do_proc_dointvec_w(&size, ctx->ctl_table, buffer, lenp, ppos,
			do_proc_dointvec_minmax_conv, SYSCTL_ZERO, &unres_qlen_max);
		...
		struct file *file, char *buffer, size_t *lenp, loff_t *ppos)
{
	...
	ret = do_proc_dointvec_r(&size, ctx->ctl_table, buffer, lenp, ppos,
