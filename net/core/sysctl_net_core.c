...
static umode_t netns_core_init_net_writable_mode(const struct ctl_context *ctx)
{
	return net_eq(ctx->ns.net_ns, &init_net) ? 0644 : 0444;
}

#define NETNS_CORE_SYSCTL_DATA(type, name)				\
static type *netns_core_##name##_data(const struct ctl_context *ctx)	\
{									\
...
static u8 *netns_core_tstamp_allow_data_value(const struct ctl_context *ctx)
{
	return &ctx->ns.net_ns->core.sysctl_tstamp_allow_data;
}

#if IS_ENABLED(CONFIG_RPS)
static void *netns_core_rps_default_mask_data(const struct ctl_context *ctx)
{
	return ctx->ns.net_ns;
}
#endif

static void *netns_core_sysctl_wmem_max_data(const struct ctl_context *ctx)
{
	return &sysctl_wmem_max;
}

static void *netns_core_sysctl_rmem_max_data(const struct ctl_context *ctx)
{
	return &sysctl_rmem_max;
}

static void *netns_core_sysctl_wmem_default_data(const struct ctl_context *ctx)
{
	return &sysctl_wmem_default;
}

static void *netns_core_sysctl_rmem_default_data(const struct ctl_context *ctx)
{
...
{
	net->core.sysctl_hdr = register_net_sysctl_fields_sz(net, "net/core",
							     netns_core_table,
