...
static void *lowpan_frag_high_thresh_data(const struct ctl_context *ctx)
{
	return &net_ieee802154_lowpan(ctx->ns.net_ns)->fqdir->high_thresh;
}

static void *lowpan_frag_low_thresh_data(const struct ctl_context *ctx)
{
	return &net_ieee802154_lowpan(ctx->ns.net_ns)->fqdir->low_thresh;
}

static void *lowpan_frag_timeout_data(const struct ctl_context *ctx)
{
...
static const struct ctl_field lowpan_frags_ns_ctl_table[] = {
	{
		...
		.data   = lowpan_frag_high_thresh_data,
		.extra1 = lowpan_frag_low_thresh_data,
	},
	{
		.table = {
		...
		.data   = lowpan_frag_low_thresh_data,
		.extra2 = lowpan_frag_high_thresh_data,
	},
	{
		.table = {
...
static int __net_init lowpan_frags_ns_sysctl_register(struct net *net)
{
	...
	hdr = register_net_sysctl_fields(net, "net/ieee802154/6lowpan",
