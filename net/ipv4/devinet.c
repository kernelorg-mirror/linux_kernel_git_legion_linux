...
static void *devinet_conf_data(const struct ctl_context *ctx)
{
	return ctx->target.ipv4_devconf;
}

static void *devinet_net_data(const struct ctl_context *ctx)
{
...
static const struct ctl_field devinet_sysctls[] = {
				     ...
				     int ifindex, struct ipv4_devconf *p)
{
	...
	hdr = register_net_sysctl_ctx(net, path, devinet_sysctls, &ctx);
	if (!hdr)
		return -ENOMEM;
...
static __net_init int devinet_init_net(struct net *net)
{
	...
	err = -ENOMEM;
	forw_ctx = (struct ctl_context) {
		.ns.net_ns = net,
		.target.ipv4_devconf = all,
	};
	forw_hdr = register_net_sysctl_ctx(net, "net/ipv4", ctl_forward_entry,
					   &forw_ctx);
	if (!forw_hdr)
		goto err_reg_ctl;
