...
static unsigned long *ip4_frags_high_thresh_data(const struct ctl_context *ctx)
{
	return &ctx->ns.net_ns->ipv4.fqdir->high_thresh;
}

static unsigned long *ip4_frags_low_thresh_data(const struct ctl_context *ctx)
{
	return &ctx->ns.net_ns->ipv4.fqdir->low_thresh;
}

static int *ip4_frags_max_dist_data(const struct ctl_context *ctx)
{
	return &ctx->ns.net_ns->ipv4.fqdir->max_dist;
}

static void *ip4_frags_timeout_data(const struct ctl_context *ctx)
{
	return &ctx->ns.net_ns->ipv4.fqdir->timeout;
}

static const struct ctl_field ip4_frags_ns_ctl_table[] = {
	CTL_FIELD_ULONG_MINMAX("ipfrag_high_thresh", 0644,
			       ip4_frags_high_thresh_data,
			       ip4_frags_low_thresh_data, NULL),
	CTL_FIELD_ULONG_MINMAX("ipfrag_low_thresh", 0644,
			       ip4_frags_low_thresh_data,
			       NULL, ip4_frags_high_thresh_data),
	CTL_FIELD_CUSTOM("ipfrag_time", 0644, sizeof(int),
			 ip4_frags_timeout_data, proc_dointvec_jiffies),
	CTL_FIELD_STATIC_INT_MINMAX("ipfrag_max_dist", 0644,
...
static int __net_init ip4_frags_ns_ctl_register(struct net *net)
{
	...
	hdr = register_net_sysctl_fields_sz(net, "net/ipv4",
