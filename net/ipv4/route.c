...
static int *ipv4_route_ ## name ## _data(const struct ctl_context *ctx)	\
{									\
	return &ctx->ns.net_ns->ipv4.name;				\
}

static void *ipv4_route_net_data(const struct ctl_context *ctx)
{
...
static void *ipv4_route_ip_rt_mtu_expires_data(const struct ctl_context *ctx)
{
	return &ctx->ns.net_ns->ipv4.ip_rt_mtu_expires;
}

static const struct ctl_field ipv4_route_netns_table[] = {
	CTL_FIELD_CUSTOM(ipv4_route_flush_procname, 0200, sizeof(int),
			 ipv4_route_net_data, ipv4_sysctl_rtcache_flush),
	CTL_FIELD_STATIC_INT_MINMAX("min_pmtu", 0644,
	...
	CTL_FIELD_INT("min_adv_mss", 0644, ipv4_route_ip_rt_min_advmss_data),
};
...
static __net_init int sysctl_route_net_init(struct net *net)
{
	...
	net->ipv4.route_hdr = register_net_sysctl_fields_sz(net, "net/ipv4/route",
