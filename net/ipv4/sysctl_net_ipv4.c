...
static umode_t ipv4_init_net_writable_mode(const struct ctl_context *ctx)
{
	return net_eq(ctx->ns.net_ns, &init_net) ? 0644 : 0444;
}

static void *ipv4_netns_data(const struct ctl_context *ctx)
{
	return ctx->ns.net_ns;
}

#define IPV4_DATA(type, name, field)					\
static type *ipv4_ ## name ## _data(const struct ctl_context *ctx)	\
{									\
...
static const struct ctl_field ipv4_net_table[] = {
	CTL_FIELD_INT("tcp_max_tw_buckets", 0644,
			ipv4_tcp_max_tw_buckets_data),
	CTL_FIELD_STATIC_U8_MINMAX("icmp_echo_ignore_all", 0644,
			ipv4_icmp_echo_ignore_all_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_STATIC_U8_MINMAX("icmp_echo_enable_probe", 0644,
			ipv4_icmp_echo_enable_probe_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_STATIC_U8_MINMAX("icmp_echo_ignore_broadcasts", 0644,
			ipv4_icmp_echo_ignore_broadcasts_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_STATIC_U8_MINMAX("icmp_ignore_bogus_error_responses", 0644,
			ipv4_icmp_ignore_bogus_error_responses_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_STATIC_U8_MINMAX("icmp_errors_use_inbound_ifaddr", 0644,
			ipv4_icmp_errors_use_inbound_ifaddr_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_STATIC_U8_MINMAX("icmp_errors_extension_mask", 0644,
			ipv4_icmp_errors_extension_mask_data,
			SYSCTL_UINT_ZERO, &icmp_errors_extension_mask_all),
	CTL_FIELD_CUSTOM("icmp_ratelimit", 0644,
			sizeof(int),
			ipv4_icmp_ratelimit_data,
			proc_dointvec_ms_jiffies),
	CTL_FIELD_INT("icmp_ratemask", 0644,
			ipv4_icmp_ratemask_data),
	CTL_FIELD_STATIC_INT_MINMAX("icmp_msgs_per_sec", 0644,
			ipv4_icmp_msgs_per_sec_data,
			SYSCTL_ZERO, NULL),
	CTL_FIELD_STATIC_INT_MINMAX("icmp_msgs_burst", 0644,
			ipv4_icmp_msgs_burst_data,
			SYSCTL_ZERO, NULL),
	CTL_FIELD_CUSTOM("ping_group_range", 0644,
			sizeof(gid_t) * 2,
			ipv4_ping_group_range_data,
			ipv4_ping_group_range),
#ifdef CONFIG_NET_L3_MASTER_DEV
	CTL_FIELD_STATIC_U8_MINMAX("raw_l3mdev_accept", 0644,
			ipv4_raw_l3mdev_accept_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
#endif
	CTL_FIELD_STATIC_U8_MINMAX("tcp_ecn", 0644,
			ipv4_tcp_ecn_data,
			SYSCTL_UINT_ZERO, &tcp_ecn_mode_max),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_ecn_option", 0644,
			ipv4_tcp_ecn_option_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_THREE),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_ecn_option_beacon", 0644,
			ipv4_tcp_ecn_option_beacon_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_THREE),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_ecn_fallback", 0644,
			ipv4_tcp_ecn_fallback_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_U8("ip_dynaddr", 0644,
			ipv4_ip_dynaddr_data),
	CTL_FIELD_U8("ip_early_demux", 0644,
			ipv4_ip_early_demux_data),
	CTL_FIELD_U8("udp_early_demux", 0644,
			ipv4_udp_early_demux_data),
	CTL_FIELD_U8("tcp_early_demux", 0644,
			ipv4_tcp_early_demux_data),
	CTL_FIELD_STATIC_U8_MINMAX("nexthop_compat_mode", 0644,
			ipv4_nexthop_compat_mode_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_STATIC_U8_MINMAX("ip_default_ttl", 0644,
			ipv4_ip_default_ttl_data,
			&ip_ttl_min, &ip_ttl_max),
	CTL_FIELD_CUSTOM("ip_local_port_range", 0644,
			0,
			ipv4_netns_data,
			ipv4_local_port_range),
	CTL_FIELD_UINT("ip_local_port_step_width", 0644,
			ipv4_ip_local_port_step_width_data),
	CTL_FIELD_CUSTOM("ip_local_reserved_ports", 0644,
			65536,
			ipv4_ip_local_reserved_ports_data,
			proc_do_large_bitmap),
	CTL_FIELD_U8("ip_no_pmtu_disc", 0644,
			ipv4_ip_no_pmtu_disc_data),
	CTL_FIELD_U8("ip_forward_use_pmtu", 0644,
			ipv4_ip_forward_use_pmtu_data),
	CTL_FIELD_CUSTOM("ip_forward_update_priority", 0644,
			sizeof(u8),
			ipv4_ip_forward_update_priority_data,
			ipv4_fwd_update_priority),
	CTL_FIELD_U8("ip_nonlocal_bind", 0644,
			ipv4_ip_nonlocal_bind_data),
	CTL_FIELD_STATIC_U8_MINMAX("ip_autobind_reuse", 0644,
			ipv4_ip_autobind_reuse_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_U8("fwmark_reflect", 0644,
			ipv4_fwmark_reflect_data),
	CTL_FIELD_U8("tcp_fwmark_accept", 0644,
			ipv4_tcp_fwmark_accept_data),
#ifdef CONFIG_NET_L3_MASTER_DEV
	CTL_FIELD_STATIC_U8_MINMAX("tcp_l3mdev_accept", 0644,
			ipv4_tcp_l3mdev_accept_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
#endif
	CTL_FIELD_U8("tcp_mtu_probing", 0644,
			ipv4_tcp_mtu_probing_data),
	CTL_FIELD_INT("tcp_base_mss", 0644,
			ipv4_tcp_base_mss_data),
	CTL_FIELD_STATIC_INT_MINMAX("tcp_min_snd_mss", 0644,
			ipv4_tcp_min_snd_mss_data,
			&tcp_min_snd_mss_min, &tcp_min_snd_mss_max),
	CTL_FIELD_STATIC_INT_MINMAX("tcp_mtu_probe_floor", 0644,
			ipv4_tcp_mtu_probe_floor_data,
			&tcp_min_snd_mss_min, &tcp_min_snd_mss_max),
	CTL_FIELD_INT("tcp_probe_threshold", 0644,
			ipv4_tcp_probe_threshold_data),
	CTL_FIELD_STATIC_UINT_MINMAX("tcp_probe_interval", 0644,
			ipv4_tcp_probe_interval_data,
			NULL, &u32_max_div_HZ),
	CTL_FIELD_U8("igmp_link_local_mcast_reports", 0644,
			ipv4_igmp_link_local_mcast_reports_data),
	CTL_FIELD_INT("igmp_max_memberships", 0644,
			ipv4_igmp_max_memberships_data),
	CTL_FIELD_INT("igmp_max_msf", 0644,
			ipv4_igmp_max_msf_data),
#ifdef CONFIG_IP_MULTICAST
	CTL_FIELD_STATIC_INT_MINMAX("igmp_qrv", 0644,
			ipv4_igmp_qrv_data,
			SYSCTL_ONE, NULL),
#endif
	CTL_FIELD_CUSTOM("tcp_congestion_control", 0644,
			TCP_CA_NAME_MAX,
			ipv4_tcp_congestion_control_data,
			proc_tcp_congestion_control),
	CTL_FIELD_CUSTOM("tcp_available_congestion_control", 0444,
			TCP_CA_BUF_MAX,
			NULL,
			proc_tcp_available_congestion_control),
	CTL_FIELD_CUSTOM_MODE("tcp_allowed_congestion_control", 0644,
			ipv4_init_net_writable_mode, TCP_CA_BUF_MAX, NULL,
			proc_allowed_congestion_control),
	CTL_FIELD_CUSTOM("tcp_keepalive_time", 0644,
			sizeof(int),
			ipv4_tcp_keepalive_time_data,
			proc_dointvec_jiffies),
	CTL_FIELD_U8("tcp_keepalive_probes", 0644,
			ipv4_tcp_keepalive_probes_data),
	CTL_FIELD_CUSTOM("tcp_keepalive_intvl", 0644,
			sizeof(int),
			ipv4_tcp_keepalive_intvl_data,
			proc_dointvec_jiffies),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_syn_retries", 0644,
			ipv4_tcp_syn_retries_data,
			&tcp_syn_retries_min, &tcp_syn_retries_max),
	CTL_FIELD_U8("tcp_synack_retries", 0644,
			ipv4_tcp_synack_retries_data),
#ifdef CONFIG_SYN_COOKIES
	CTL_FIELD_U8("tcp_syncookies", 0644,
			ipv4_tcp_syncookies_data),
#endif
	CTL_FIELD_STATIC_U8_MINMAX("tcp_migrate_req", 0644,
			ipv4_tcp_migrate_req_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_INT("tcp_reordering", 0644,
			ipv4_tcp_reordering_data),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_retries1", 0644,
			ipv4_tcp_retries1_data,
			NULL, &tcp_retr1_max),
	CTL_FIELD_U8("tcp_retries2", 0644,
			ipv4_tcp_retries2_data),
	CTL_FIELD_U8("tcp_orphan_retries", 0644,
			ipv4_tcp_orphan_retries_data),
	CTL_FIELD_CUSTOM("tcp_fin_timeout", 0644,
			sizeof(int),
			ipv4_tcp_fin_timeout_data,
			proc_dointvec_jiffies),
	CTL_FIELD_UINT("tcp_notsent_lowat", 0644,
			ipv4_tcp_notsent_lowat_data),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_tw_reuse", 0644,
			ipv4_tcp_tw_reuse_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_TWO),
	CTL_FIELD_STATIC_UINT_MINMAX("tcp_tw_reuse_delay", 0644,
			ipv4_tcp_tw_reuse_delay_data,
			SYSCTL_UINT_ONE, &tcp_tw_reuse_delay_max),
	CTL_FIELD_INT("tcp_max_syn_backlog", 0644,
			ipv4_tcp_max_syn_backlog_data),
	CTL_FIELD_INT("tcp_fastopen", 0644,
			ipv4_tcp_fastopen_data),
	CTL_FIELD_CUSTOM("tcp_fastopen_key", 0600,
			((TCP_FASTOPEN_KEY_LENGTH * 2 * TCP_FASTOPEN_KEY_MAX) + (TCP_FASTOPEN_KEY_MAX * 5)),
			ipv4_tcp_fastopen_key_data,
			proc_tcp_fastopen_key),
	CTL_FIELD_CUSTOM("tcp_fastopen_blackhole_timeout_sec", 0644,
			sizeof(int),
			ipv4_tcp_fastopen_blackhole_timeout_sec_data,
			proc_tfo_blackhole_detect_timeout),
#ifdef CONFIG_IP_ROUTE_MULTIPATH
	CTL_FIELD_STATIC_U8_MINMAX("fib_multipath_use_neigh", 0644,
			ipv4_fib_multipath_use_neigh_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_CUSTOM("fib_multipath_hash_policy", 0644,
			sizeof(u8),
			ipv4_fib_multipath_hash_policy_data,
			proc_fib_multipath_hash_policy),
	CTL_FIELD_CUSTOM("fib_multipath_hash_fields", 0644,
			sizeof(u32),
			ipv4_fib_multipath_hash_fields_data,
			proc_fib_multipath_hash_fields),
	CTL_FIELD_CUSTOM("fib_multipath_hash_seed", 0644,
			sizeof(u32),
			ipv4_netns_data,
			proc_fib_multipath_hash_seed),
#endif
	CTL_FIELD_CUSTOM("ip_unprivileged_port_start", 0644,
			sizeof(int),
			ipv4_ip_unprivileged_port_start_data,
			ipv4_privileged_ports),
#ifdef CONFIG_NET_L3_MASTER_DEV
	CTL_FIELD_STATIC_U8_MINMAX("udp_l3mdev_accept", 0644,
			ipv4_udp_l3mdev_accept_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
#endif
	CTL_FIELD_U8("tcp_sack", 0644,
			ipv4_tcp_sack_data),
	CTL_FIELD_U8("tcp_window_scaling", 0644,
			ipv4_tcp_window_scaling_data),
	CTL_FIELD_U8("tcp_timestamps", 0644,
			ipv4_tcp_timestamps_data),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_early_retrans", 0644,
			ipv4_tcp_early_retrans_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_FOUR),
	CTL_FIELD_U8("tcp_recovery", 0644,
			ipv4_tcp_recovery_data),
	CTL_FIELD_U8("tcp_thin_linear_timeouts", 0644,
			ipv4_tcp_thin_linear_timeouts_data),
	CTL_FIELD_U8("tcp_slow_start_after_idle", 0644,
			ipv4_tcp_slow_start_after_idle_data),
	CTL_FIELD_U8("tcp_retrans_collapse", 0644,
			ipv4_tcp_retrans_collapse_data),
	CTL_FIELD_U8("tcp_stdurg", 0644,
			ipv4_tcp_stdurg_data),
	CTL_FIELD_U8("tcp_rfc1337", 0644,
			ipv4_tcp_rfc1337_data),
	CTL_FIELD_U8("tcp_abort_on_overflow", 0644,
			ipv4_tcp_abort_on_overflow_data),
	CTL_FIELD_U8("tcp_fack", 0644,
			ipv4_tcp_fack_data),
	CTL_FIELD_INT("tcp_max_reordering", 0644,
			ipv4_tcp_max_reordering_data),
	CTL_FIELD_U8("tcp_dsack", 0644,
			ipv4_tcp_dsack_data),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_app_win", 0644,
			ipv4_tcp_app_win_data,
			SYSCTL_UINT_ZERO, &tcp_app_win_max),
	CTL_FIELD_STATIC_INT_MINMAX("tcp_adv_win_scale", 0644,
			ipv4_tcp_adv_win_scale_data,
			&tcp_adv_win_scale_min, &tcp_adv_win_scale_max),
	CTL_FIELD_U8("tcp_frto", 0644,
			ipv4_tcp_frto_data),
	CTL_FIELD_U8("tcp_no_metrics_save", 0644,
			ipv4_tcp_no_metrics_save_data),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_no_ssthresh_metrics_save", 0644,
			ipv4_tcp_no_ssthresh_metrics_save_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_U8("tcp_moderate_rcvbuf", 0644,
			ipv4_tcp_moderate_rcvbuf_data),
	CTL_FIELD_STATIC_INT_MINMAX("tcp_rcvbuf_low_rtt", 0644,
			ipv4_tcp_rcvbuf_low_rtt_data,
			SYSCTL_ZERO, SYSCTL_INT_MAX),
	CTL_FIELD_U8("tcp_tso_win_divisor", 0644,
			ipv4_tcp_tso_win_divisor_data),
	CTL_FIELD_U8("tcp_workaround_signed_windows", 0644,
			ipv4_tcp_workaround_signed_windows_data),
	CTL_FIELD_INT("tcp_limit_output_bytes", 0644,
			ipv4_tcp_limit_output_bytes_data),
	CTL_FIELD_INT("tcp_challenge_ack_limit", 0644,
			ipv4_tcp_challenge_ack_limit_data),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_min_tso_segs", 0644,
			ipv4_tcp_min_tso_segs_data,
			SYSCTL_UINT_ONE, NULL),
	CTL_FIELD_U8("tcp_tso_rtt_log", 0644,
			ipv4_tcp_tso_rtt_log_data),
	CTL_FIELD_STATIC_INT_MINMAX("tcp_min_rtt_wlen", 0644,
			ipv4_tcp_min_rtt_wlen_data,
			SYSCTL_ZERO, &one_day_secs),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_autocorking", 0644,
			ipv4_tcp_autocorking_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_CUSTOM("tcp_invalid_ratelimit", 0644,
			sizeof(int),
			ipv4_tcp_invalid_ratelimit_data,
			proc_dointvec_ms_jiffies),
	CTL_FIELD_STATIC_INT_MINMAX("tcp_pacing_ss_ratio", 0644,
			ipv4_tcp_pacing_ss_ratio_data,
			SYSCTL_ZERO, SYSCTL_ONE_THOUSAND),
	CTL_FIELD_STATIC_INT_MINMAX("tcp_pacing_ca_ratio", 0644,
			ipv4_tcp_pacing_ca_ratio_data,
			SYSCTL_ZERO, SYSCTL_ONE_THOUSAND),
	CTL_FIELD_CUSTOM("tcp_wmem", 0644,
			sizeof(init_net.ipv4.sysctl_tcp_wmem),
			ipv4_tcp_wmem_data,
			proc_dointvec_minmax_one),
	CTL_FIELD_CUSTOM("tcp_rmem", 0644,
			sizeof(init_net.ipv4.sysctl_tcp_rmem),
			ipv4_tcp_rmem_data,
			proc_dointvec_minmax_one),
	CTL_FIELD_ULONG("tcp_comp_sack_delay_ns", 0644,
			ipv4_tcp_comp_sack_delay_ns_data),
	CTL_FIELD_STATIC_INT_MINMAX("tcp_comp_sack_rtt_percent", 0644,
			ipv4_tcp_comp_sack_rtt_percent_data,
			SYSCTL_ONE, SYSCTL_ONE_THOUSAND),
	CTL_FIELD_ULONG("tcp_comp_sack_slack_ns", 0644,
			ipv4_tcp_comp_sack_slack_ns_data),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_comp_sack_nr", 0644,
			ipv4_tcp_comp_sack_nr_data,
			SYSCTL_UINT_ZERO, NULL),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_backlog_ack_defer", 0644,
			ipv4_tcp_backlog_ack_defer_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_reflect_tos", 0644,
			ipv4_tcp_reflect_tos_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_CUSTOM("tcp_ehash_entries", 0444,
			0,
			ipv4_tcp_ehash_entries_data,
			proc_tcp_ehash_entries),
	CTL_FIELD_STATIC_UINT_MINMAX("tcp_child_ehash_entries", 0644,
			ipv4_tcp_child_ehash_entries_data,
			SYSCTL_UINT_ZERO, &tcp_child_ehash_entries_max),
	CTL_FIELD_CUSTOM("udp_hash_entries", 0444,
			0,
			ipv4_udp_hash_entries_data,
			proc_udp_hash_entries),
	CTL_FIELD_STATIC_UINT_MINMAX("udp_child_hash_entries", 0644,
			ipv4_udp_child_hash_entries_data,
			SYSCTL_UINT_ZERO, &udp_child_hash_entries_max),
	CTL_FIELD_STATIC_INT_MINMAX("udp_rmem_min", 0644,
			ipv4_udp_rmem_min_data, SYSCTL_ONE, NULL),
	CTL_FIELD_STATIC_INT_MINMAX("udp_wmem_min", 0644,
			ipv4_udp_wmem_min_data, SYSCTL_ONE, NULL),
	CTL_FIELD_STATIC_U8_MINMAX("fib_notify_on_flag_change", 0644,
			ipv4_fib_notify_on_flag_change_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_TWO),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_plb_enabled", 0644,
			ipv4_tcp_plb_enabled_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_plb_idle_rehash_rounds", 0644,
			ipv4_tcp_plb_idle_rehash_rounds_data,
			NULL, &tcp_plb_max_rounds),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_plb_rehash_rounds", 0644,
			ipv4_tcp_plb_rehash_rounds_data,
			NULL, &tcp_plb_max_rounds),
	CTL_FIELD_U8("tcp_plb_suspend_rto_sec", 0644,
			ipv4_tcp_plb_suspend_rto_sec_data),
	CTL_FIELD_STATIC_INT_MINMAX("tcp_plb_cong_thresh", 0644,
			ipv4_tcp_plb_cong_thresh_data,
			SYSCTL_ZERO, &tcp_plb_max_cong_thresh),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_syn_linear_timeouts", 0644,
			ipv4_tcp_syn_linear_timeouts_data,
			SYSCTL_UINT_ZERO, &tcp_syn_linear_timeouts_max),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_shrink_window", 0644,
			ipv4_tcp_shrink_window_data,
			SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_STATIC_U8_MINMAX("tcp_pingpong_thresh", 0644,
			ipv4_tcp_pingpong_thresh_data,
			SYSCTL_UINT_ONE, NULL),
	CTL_FIELD_STATIC_INT_MINMAX("tcp_rto_min_us", 0644,
			ipv4_tcp_rto_min_us_data,
			SYSCTL_ONE, NULL),
	CTL_FIELD_STATIC_INT_MINMAX("tcp_rto_max_ms", 0644,
...
static __net_init int ipv4_sysctl_init_net(struct net *net)
{
	...
	net->ipv4.ipv4_hdr = register_net_sysctl_fields_sz(net, "net/ipv4",
							   ipv4_net_table,
							   table_size);
