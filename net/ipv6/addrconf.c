...
static type *addrconf_ ## field ## _data(const struct ctl_context *ctx)	\
{									\
	return &ctx->target.ipv6_devconf->field;			\
}

static void *addrconf_idev_data(const struct ctl_context *ctx)
{
	struct ipv6_devconf *cnf = ctx->target.ipv6_devconf;
...
static void *addrconf_net_data(const struct ctl_context *ctx)
{
...
#define ADDRCONF_INT_ENTRY(field, name) \
	CTL_FIELD_INT(name, 0644, addrconf_ ## field ## _data)

#define ADDRCONF_INT_RO_ENTRY(field, name) \
	CTL_FIELD_INT(name, 0444, addrconf_ ## field ## _data)

#define ADDRCONF_INT_MINMAX_ENTRY(field, name, min, max)		\
	CTL_FIELD_STATIC_INT_MINMAX(name, 0644, addrconf_ ## field ## _data, \
				    min, max)

#define ADDRCONF_UINT_ENTRY(field, name) \
	CTL_FIELD_UINT(name, 0644, addrconf_ ## field ## _data)

#define ADDRCONF_UINT_MINMAX_ENTRY(field, name, min, max)		\
	CTL_FIELD_STATIC_UINT_MINMAX(name, 0644, addrconf_ ## field ## _data, \
				     min, max)

#define ADDRCONF_U8_MINMAX_ENTRY(field, name, min, max)			\
	CTL_FIELD_STATIC_U8_MINMAX(name, 0644, addrconf_ ## field ## _data, \
				   min, max)
		...
		struct inet6_dev *idev, struct ipv6_devconf *p)
{
	...
	p->sysctl_header = register_net_sysctl_ctx(net, path, addrconf_sysctl,
						   &ctx);
	if (!p->sysctl_header)
		goto out;
