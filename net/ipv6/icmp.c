...
static type *ipv6_icmp_##name##_data(const struct ctl_context *ctx)	\
{									\
	return &ctx->ns.net_ns->ipv6.sysctl.field;			\
}

#define IPV6_ICMP_CUSTOM_DATA(name, field)				\
static void *ipv6_icmp_##name##_data(const struct ctl_context *ctx)	\
{									\
	return &ctx->ns.net_ns->ipv6.sysctl.field;			\
}
...
static const struct ctl_field ipv6_icmp_table[] = {
	CTL_FIELD_CUSTOM("ratelimit", 0644, sizeof(int),
			 ipv6_icmp_ratelimit_data, proc_dointvec_ms_jiffies),
	CTL_FIELD_U8("echo_ignore_all", 0644,
		     ipv6_icmp_echo_ignore_all_data),
	CTL_FIELD_U8("echo_ignore_multicast", 0644,
		     ipv6_icmp_echo_ignore_multicast_data),
	CTL_FIELD_U8("echo_ignore_anycast", 0644,
		     ipv6_icmp_echo_ignore_anycast_data),
	CTL_FIELD_CUSTOM("ratemask", 0644, ICMPV6_MSG_MAX + 1,
			 ipv6_icmp_ratemask_data, proc_do_large_bitmap),
	CTL_FIELD_STATIC_U8_MINMAX("error_anycast_as_unicast", 0644,
				   ipv6_icmp_error_anycast_as_unicast_data,
				   SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_STATIC_U8_MINMAX("errors_extension_mask", 0644,
...
{
	return ipv6_icmp_table;
}

size_t ipv6_icmp_sysctl_table_size(void)
{
	return ARRAY_SIZE(ipv6_icmp_table);
}
#endif
