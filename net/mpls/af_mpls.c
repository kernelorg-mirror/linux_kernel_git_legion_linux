		...
		struct file *file, char *buffer, size_t *lenp, loff_t *ppos)
{
	...
	ssize_t ret = ret = do_proc_dointvec_w(ctx->ctl_table->data, ctx->ctl_table,
			buffer, lenp, ppos,
		...
		char *buffer, size_t *lenp, loff_t *ppos)
{
	...
	ret = do_proc_dointvec_w(&platform_labels, ctx->ctl_table, buffer, lenp, ppos,
			do_proc_dointvec_minmax_conv, SYSCTL_ZERO, &label_limit);
		...
		char *buffer, size_t *lenp, loff_t *ppos)
{
	...
	return do_proc_dointvec_r(&platform_labels, ctx->ctl_table, buffer, lenp, ppos,
			do_proc_dointvec_minmax_conv, NULL, NULL);
}
