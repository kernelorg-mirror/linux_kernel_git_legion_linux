...
static type *mptcp_ ## field ## _data(const struct ctl_context *ctx)	\
{									\
	return &mptcp_get_pernet(ctx->ns.net_ns)->field;		\
}

#define MPTCP_CUSTOM_DATA(field)					\
static void *mptcp_ ## field ## _data(const struct ctl_context *ctx)	\
{									\
	return &mptcp_get_pernet(ctx->ns.net_ns)->field;		\
}

static u8 *mptcp_enabled_data(const struct ctl_context *ctx)
{
...
static const struct ctl_field mptcp_sysctl_table[] = {
	 ...
	 */
	CTL_FIELD_STATIC_U8_MINMAX("enabled", 0644, mptcp_enabled_data,
				   SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_CUSTOM("add_addr_timeout", 0644, sizeof(unsigned int),
			 mptcp_add_addr_timeout_data, proc_dointvec_jiffies),
	CTL_FIELD_STATIC_U8_MINMAX("checksum_enabled", 0644,
				   mptcp_checksum_enabled_data,
				   SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_STATIC_U8_MINMAX("allow_join_initial_addr_port", 0644,
				   mptcp_allow_join_initial_addr_port_data,
				   SYSCTL_UINT_ZERO, SYSCTL_UINT_ONE),
	CTL_FIELD_UINT("stale_loss_cnt", 0644, mptcp_stale_loss_cnt_data),
	CTL_FIELD_CUSTOM("pm_type", 0644, sizeof(u8),
			 mptcp_pm_type_data, proc_pm_type),
	CTL_FIELD_CUSTOM("scheduler", 0644, MPTCP_SCHED_NAME_MAX,
			 mptcp_scheduler_data, proc_scheduler),
	CTL_FIELD_CUSTOM("available_schedulers", 0444, MPTCP_SCHED_BUF_MAX,
			 NULL, proc_available_schedulers),
	CTL_FIELD_CUSTOM("close_timeout", 0644, sizeof(unsigned int),
			 mptcp_close_timeout_data, proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("blackhole_timeout", 0644, sizeof(unsigned int),
			 mptcp_blackhole_timeout_data,
			 proc_blackhole_detect_timeout),
	CTL_FIELD_U8("syn_retrans_before_tcp_fallback", 0644,
		     mptcp_syn_retrans_before_tcp_fallback_data),
	CTL_FIELD_CUSTOM("path_manager", 0644, MPTCP_PM_NAME_MAX,
			 mptcp_path_manager_data, proc_path_manager),
	CTL_FIELD_CUSTOM("available_path_managers", 0444, MPTCP_PM_BUF_MAX,
...
static int mptcp_pernet_new_table(struct net *net, struct mptcp_pernet *pernet)
{
	...
	hdr = register_net_sysctl_fields_sz(net, MPTCP_SYSCTL_PATH,
					    mptcp_sysctl_table,
