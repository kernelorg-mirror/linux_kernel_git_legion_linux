...
static umode_t nf_ct_global_sysctl_mode(const struct ctl_context *ctx)
{
	return net_eq(ctx->ns.net_ns, &init_net) ? 0644 : 0444;
}

static int *nf_ct_max_data(const struct ctl_context *ctx)
{
	return &nf_conntrack_max;
}

static void *nf_ct_count_data(const struct ctl_context *ctx)
{
...
static void *nf_ct_buckets_data(const struct ctl_context *ctx)
{
	return &nf_conntrack_htable_size_user;
}

static u8 *nf_ct_checksum_data(const struct ctl_context *ctx)
{
	return &ctx->ns.net_ns->ct.sysctl_checksum;
}

static void *nf_ct_log_invalid_data(const struct ctl_context *ctx)
{
	return &ctx->ns.net_ns->ct.sysctl_log_invalid;
}

static int *nf_ct_expect_max_data(const struct ctl_context *ctx)
{
	return &nf_ct_expect_max;
}

static u8 *nf_ct_acct_data(const struct ctl_context *ctx)
{
	return &ctx->ns.net_ns->ct.sysctl_acct;
}

#ifdef CONFIG_NF_CONNTRACK_EVENTS
static u8 *nf_ct_events_data(const struct ctl_context *ctx)
{
...
#ifdef CONFIG_NF_CONNTRACK_TIMESTAMP
static u8 *nf_ct_timestamp_data(const struct ctl_context *ctx)
{
	return &ctx->ns.net_ns->ct.sysctl_tstamp;
}
#endif

static void *nf_ct_generic_timeout_data(const struct ctl_context *ctx)
{
...
static void *nf_ct_tcp_offload_timeout_data(const struct ctl_context *ctx)
{
...
static u8 *nf_ct_tcp_ ## name ## _data(const struct ctl_context *ctx)	\
{									\
...
static void *nf_ct_udp_timeout_data(const struct ctl_context *ctx)
{
	return &nf_udp_pernet(ctx->ns.net_ns)->timeouts[UDP_CT_UNREPLIED];
}

static void *nf_ct_udp_stream_timeout_data(const struct ctl_context *ctx)
{
	return &nf_udp_pernet(ctx->ns.net_ns)->timeouts[UDP_CT_REPLIED];
}

#if IS_ENABLED(CONFIG_NF_FLOW_TABLE)
static void *nf_ct_udp_offload_timeout_data(const struct ctl_context *ctx)
{
	return &nf_udp_pernet(ctx->ns.net_ns)->offload_timeout;
}
#endif

static void *nf_ct_icmp_timeout_data(const struct ctl_context *ctx)
{
	return &nf_icmp_pernet(ctx->ns.net_ns)->timeout;
}

static void *nf_ct_icmpv6_timeout_data(const struct ctl_context *ctx)
{
...
static void *nf_ct_gre_timeout_data(const struct ctl_context *ctx)
{
	return &nf_gre_pernet(ctx->ns.net_ns)->timeouts[GRE_CT_UNREPLIED];
}

static void *nf_ct_gre_stream_timeout_data(const struct ctl_context *ctx)
{
	return &nf_gre_pernet(ctx->ns.net_ns)->timeouts[GRE_CT_REPLIED];
}
#endif

#define NF_CT_GLOBAL_INT_MINMAX(_procname, _data)			\
	{								\
		.procname	= (_procname),				\
		.mode		= 0644,					\
		.mode_fn	= nf_ct_global_sysctl_mode,		\
		.type		= CTL_FIELD_STATIC_INT_MINMAX,		\
		.ctl_static_int	= {					\
			.data		= (_data),			\
...
static const struct ctl_field nf_ct_sysctl_table[] = {
	NF_CT_GLOBAL_INT_MINMAX("nf_conntrack_max", nf_ct_max_data),
	CTL_FIELD_CUSTOM("nf_conntrack_count", 0444, sizeof(int),
			 nf_ct_count_data, proc_dointvec),
	CTL_FIELD_CUSTOM_MODE("nf_conntrack_buckets", 0644,
			      nf_ct_global_sysctl_mode,
			      sizeof(unsigned int),
			      nf_ct_buckets_data,
			      nf_conntrack_hash_sysctl),
	CTL_FIELD_STATIC_U8_MINMAX("nf_conntrack_checksum", 0644,
				   nf_ct_checksum_data, SYSCTL_UINT_ZERO,
				   SYSCTL_UINT_ONE),
	CTL_FIELD_CUSTOM("nf_conntrack_log_invalid", 0644, sizeof(u8),
			 nf_ct_log_invalid_data,
			 nf_conntrack_log_invalid_sysctl),
	NF_CT_GLOBAL_INT_MINMAX("nf_conntrack_expect_max",
				nf_ct_expect_max_data),
	CTL_FIELD_STATIC_U8_MINMAX("nf_conntrack_acct", 0644,
				   nf_ct_acct_data, SYSCTL_UINT_ZERO,
				   SYSCTL_UINT_ONE),
#ifdef CONFIG_NF_CONNTRACK_EVENTS
	CTL_FIELD_STATIC_U8_MINMAX("nf_conntrack_events", 0644,
				   nf_ct_events_data, SYSCTL_UINT_ZERO,
				   SYSCTL_UINT_TWO),
#endif
#ifdef CONFIG_NF_CONNTRACK_TIMESTAMP
	CTL_FIELD_STATIC_U8_MINMAX("nf_conntrack_timestamp", 0644,
				   nf_ct_timestamp_data,
				   SYSCTL_UINT_ZERO,
				   SYSCTL_UINT_ONE),
#endif
	CTL_FIELD_CUSTOM("nf_conntrack_generic_timeout", 0644,
			 sizeof(unsigned int),
			 nf_ct_generic_timeout_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_tcp_timeout_syn_sent", 0644,
			 sizeof(unsigned int),
			 nf_ct_tcp_timeout_syn_sent_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_tcp_timeout_syn_recv", 0644,
			 sizeof(unsigned int),
			 nf_ct_tcp_timeout_syn_recv_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_tcp_timeout_established", 0644,
			 sizeof(unsigned int),
			 nf_ct_tcp_timeout_established_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_tcp_timeout_fin_wait", 0644,
			 sizeof(unsigned int),
			 nf_ct_tcp_timeout_fin_wait_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_tcp_timeout_close_wait", 0644,
			 sizeof(unsigned int),
			 nf_ct_tcp_timeout_close_wait_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_tcp_timeout_last_ack", 0644,
			 sizeof(unsigned int),
			 nf_ct_tcp_timeout_last_ack_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_tcp_timeout_time_wait", 0644,
			 sizeof(unsigned int),
			 nf_ct_tcp_timeout_time_wait_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_tcp_timeout_close", 0644,
			 sizeof(unsigned int),
			 nf_ct_tcp_timeout_close_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_tcp_timeout_max_retrans", 0644,
			 sizeof(unsigned int),
			 nf_ct_tcp_timeout_retrans_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_tcp_timeout_unacknowledged", 0644,
			 sizeof(unsigned int),
			 nf_ct_tcp_timeout_unack_data,
			 proc_dointvec_jiffies),
#if IS_ENABLED(CONFIG_NF_FLOW_TABLE)
	CTL_FIELD_CUSTOM("nf_flowtable_tcp_timeout", 0644,
			 sizeof(unsigned int),
			 nf_ct_tcp_offload_timeout_data,
			 proc_dointvec_jiffies),
#endif
	CTL_FIELD_STATIC_U8_MINMAX("nf_conntrack_tcp_loose", 0644,
				   nf_ct_tcp_loose_data,
				   SYSCTL_UINT_ZERO,
				   SYSCTL_UINT_ONE),
	CTL_FIELD_STATIC_U8_MINMAX("nf_conntrack_tcp_be_liberal",
				   0644, nf_ct_tcp_liberal_data,
				   SYSCTL_UINT_ZERO,
				   SYSCTL_UINT_ONE),
	CTL_FIELD_STATIC_U8_MINMAX("nf_conntrack_tcp_ignore_invalid_rst",
				   0644,
				   nf_ct_tcp_ignore_invalid_rst_data,
				   SYSCTL_UINT_ZERO,
				   SYSCTL_UINT_ONE),
	CTL_FIELD_U8("nf_conntrack_tcp_max_retrans", 0644,
			     nf_ct_tcp_max_retrans_data),
	CTL_FIELD_CUSTOM("nf_conntrack_udp_timeout", 0644,
			 sizeof(unsigned int),
			 nf_ct_udp_timeout_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_udp_timeout_stream", 0644,
			 sizeof(unsigned int),
			 nf_ct_udp_stream_timeout_data,
			 proc_dointvec_jiffies),
#if IS_ENABLED(CONFIG_NF_FLOW_TABLE)
	CTL_FIELD_CUSTOM("nf_flowtable_udp_timeout", 0644,
			 sizeof(unsigned int),
			 nf_ct_udp_offload_timeout_data,
			 proc_dointvec_jiffies),
#endif
	CTL_FIELD_CUSTOM("nf_conntrack_icmp_timeout", 0644,
			 sizeof(unsigned int),
			 nf_ct_icmp_timeout_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_icmpv6_timeout", 0644,
			 sizeof(unsigned int),
			 nf_ct_icmpv6_timeout_data,
			 proc_dointvec_jiffies),
#ifdef CONFIG_NF_CT_PROTO_SCTP
	CTL_FIELD_CUSTOM("nf_conntrack_sctp_timeout_closed", 0644,
			 sizeof(unsigned int),
			 nf_ct_sctp_timeout_closed_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_sctp_timeout_cookie_wait", 0644,
			 sizeof(unsigned int),
			 nf_ct_sctp_timeout_cookie_wait_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_sctp_timeout_cookie_echoed",
			 0644, sizeof(unsigned int),
			 nf_ct_sctp_timeout_cookie_echoed_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_sctp_timeout_established",
			 0644, sizeof(unsigned int),
			 nf_ct_sctp_timeout_established_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_sctp_timeout_shutdown_sent",
			 0644, sizeof(unsigned int),
			 nf_ct_sctp_timeout_shutdown_sent_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_sctp_timeout_shutdown_recd",
			 0644, sizeof(unsigned int),
			 nf_ct_sctp_timeout_shutdown_recd_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_sctp_timeout_shutdown_ack_sent",
			 0644, sizeof(unsigned int),
			 nf_ct_sctp_timeout_shutdown_ack_sent_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_sctp_timeout_heartbeat_sent",
			 0644, sizeof(unsigned int),
			 nf_ct_sctp_timeout_heartbeat_sent_data,
			 proc_dointvec_jiffies),
#endif
#ifdef CONFIG_NF_CT_PROTO_GRE
	CTL_FIELD_CUSTOM("nf_conntrack_gre_timeout", 0644,
			 sizeof(unsigned int),
			 nf_ct_gre_timeout_data,
			 proc_dointvec_jiffies),
	CTL_FIELD_CUSTOM("nf_conntrack_gre_timeout_stream", 0644,
			 sizeof(unsigned int),
			 nf_ct_gre_stream_timeout_data,
			 proc_dointvec_jiffies),
#endif
};
#undef NF_CT_GLOBAL_INT_MINMAX
...
static int nf_conntrack_standalone_init_sysctl(struct net *net)
{
	...
	cnet->sysctl_header =
		register_net_sysctl_fields_sz(net, "net/netfilter",
				      nf_ct_sysctl_table,
